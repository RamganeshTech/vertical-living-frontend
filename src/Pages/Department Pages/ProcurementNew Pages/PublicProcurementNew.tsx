...
import { 
    ...
    usePublicUpdateProcurementItemRateNewVersion, 
...
const PublicProcurementRatePage = () => {
    ...
    const orderId = searchParams.get("orderId") || ""; //this represent the procurement dept _id itself
    const quoteId = searchParams.get("quoteId") || ""; //this represent the particulr quoteid 

    // --- API Hooks ---
    // 1. Get Data
    const { data: apiData, isLoading, isError } = useGetProcurementItemsPublic(token || "", orderId, quoteId);
    
    // 2. Single Item Update (Background Auto-Save)
    // const { mutate: updateSingleItem } = usePublicUpdateProcurementItemRate();
    const { mutate: updateSingleItemNewVersion } = usePublicUpdateProcurementItemRateNewVersion();
    ...
    const handleSingleItemSave = (itemId: string, rate: number) => {
        // We use the new mutation which maps to your v1 controller
        updateSingleItemNewVersion(
            { 
                token, 
                orderId, 
                quoteId, // Passed here
                itemId, 
                rate // Backend v1 controller expects Number in body
            },
            {
                onSuccess: () => {
                    // Update local UI state to stop showing "Saving..."
                    setItems((prev) => 
                        prev.map(i => i._id === itemId ? { ...i, isSaving: false } : i)
                    );
                },
                onError: (error: any) => {
