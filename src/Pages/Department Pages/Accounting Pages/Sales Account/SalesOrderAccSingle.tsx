// SalesOrderAccSingle.tsx
import { useParams, useNavigate } from 'react-router-dom';
import { toast } from '../../../../utils/toast';
import SalesOrderAccountForm from './SalesOrderAccountForm';
import { useGetSingleSalesOrder } from '../../../../apiList/Department Api/Accounting Api/salesOrderApi';
import type { CreateSalesOrderPayload } from './CreateSalerOrderAcc';

const SalesOrderAccSingle = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const { organizationId } = useParams() as { organizationId: string }
    const { data: salesOrder, isLoading, isError, error } = useGetSingleSalesOrder(id || '');

    const handleUpdate = async (_data: CreateSalesOrderPayload) => {
        try {
            ...
            toast({
                title: "Error",
                description: error?.response?.data?.message || error?.message || "Failed to update the ordeer",
                variant: "destructive"
            });
        }
    };
...
export default SalesOrderAccSingle;