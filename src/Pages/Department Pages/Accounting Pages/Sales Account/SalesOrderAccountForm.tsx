...
interface SalesOrderAccountFormProps {
    mode: 'create' | 'view' | 'edit';
    initialData?: any;
    onSubmit: (data: CreateSalesOrderPayload) => Promise<void>;
    isSubmitting: boolean;
    organizationId: string;
}

const SalesOrderAccountForm: React.FC<SalesOrderAccountFormProps> = ({
    mode: initialMode,
    initialData,
    onSubmit,
    isSubmitting,
    ...
    const defaultFormData: SalesOrderFormData = {
        customerId: '',
        customerName: '',
        expectedShipmentDate: new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
    ...
    useEffect(() => {
        if (initialData) {
            setFormData({
                ...
                expectedShipmentDate: initialData.expectedShipmentDate || '',
    ...
    const validateForm = (formData: SalesOrderFormData): string[] => {
        const errors: string[] = [];
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload: CreateSalesOrderPayload = {
            ...dataToValidate,
            organizationId: '', // Will be set in CreateOrderAcc component
...
export default SalesOrderAccountForm;