...
const AccountingSingle: React.FC = () => {
    ...
    const isInvoiceType = typeStr.includes('invoice');
    const isExpenseType = typeStr.includes('expense');

    // 1. Show Source Item Table? (Yes for Bills/Invoices, No for Expenses)
    const showSourceTable = !isExpenseType;

    // 2. Show Payment Breakdown Table? (Yes for Bills/Expenses, No for Invoices)
    const showPaymentTable = !isInvoiceType;
    ...
    return (
        <div className="p-4 h-full overflow-y-auto custom-scrollbar space-y-6 bg-brand-main ">
            ...
            {/* --- Header --- */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-ash-medium pb-4">
                <div className="flex gap-3 items-center">
                    <button
                        onClick={() => navigate(-1)}
                        className="bg-brand-surface hover:bg-brand-surface-hover shadow-sm flex items-center justify-center w-10 h-10 border border-ash-medium text-text-main rounded-lg transition-all"
                    >
                        <i className="fas fa-arrow-left" />
                    </button>
                    <div>
                ...
                <Card className="shadow-sm border-ash-light bg-brand-surface">
                    <CardContent className="p-5">
                        <div className="flex items-center justify-between mb-2">
                            ...
                            <i className={`fas ${isInvoiceType ? 'fa-file-contract' : isExpenseType ? 'fa-receipt' : 'fa-file-invoice'} text-action-primary text-xl`}></i>
                        </div>
            ...
            {showSourceTable && (
                <Card className="shadow-sm border-ash-light overflow-hidden bg-brand-surface">
                    <div className="px-6 py-4 border-b border-ash-medium bg-brand-surface-hover flex justify-between items-center">
                        ...
                        <span className="px-3 py-1 bg-brand-ash border border-ash-medium rounded-full text-xs font-medium text-gray-600">
                            {source?.items?.length || 0} Items
                        </span>
                    </div>
            ...
            {/* ========================================================= */}
            {showPaymentTable && (
                <Card className="shadow-sm border-ash-light overflow-hidden">
                    <div className="px-6 py-4 border-b border-ash-medium bg-brand-surface-hover flex justify-between items-center">
                        <h2 className="font-bold text-text-main flex items-center gap-2">
                            <i className="fas fa-money-bill-wave text-action-primary"></i>
                            Payment Transactions
                        </h2>
                        <span className="px-3 py-1 bg-brand-ash border border-ash-medium rounded-full text-xs font-medium text-action-primary">
                                ...
                                {(!payment?.items || payment.items.length === 0) ? (
                                    ...
                                    payment.items.map((pItem: any, index: number) => (
                                        <tr key={pItem._id || index} className="hover:bg-brand-surface-hover transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                {pItem.orderId ? (
                                                    <span className="font-mono text-[10px] bg-brand-ash px-2 py-1 rounded border border-ash-medium text-text-main block truncate w-28" title={pItem.orderId}>
                                                        {pItem.orderId}
                                                    </span>
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                {pItem.status === 'paid' ? (
                                                    <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full border border-action-success text-action-successtext-[10px] font-bold uppercase">
            ...
            {!isExpenseType && source?.notes && (
                <div className="bg-brand-ash border border-action-warning rounded-lg p-4 mt-4">
