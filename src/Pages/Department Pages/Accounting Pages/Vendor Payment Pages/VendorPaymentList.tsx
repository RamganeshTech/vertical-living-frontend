...
const VendorPaymentList: React.FC<Props> = ({ vendor, index, handleView, handleDelete, deletePending }) => {
    ...
    //             {/* ₹{vendor.grandTotal?.toFixed(2) || '0.00'} */}
    //             ₹{vendor.totalAmount?.toFixed(2) || '0.00'}
    //         </div>
    ...
    return (
        ...
        >
            ...
            <div className="col-span-3">
                <div className="flex items-center gap-2">
                    <i className="fas fa-user text-action-primary text-sm"></i>
            ...
            {/* vendorOrderNumber Number */}
            <div className="col-span-3">
                <span className="px-3 py-1 bg-brand-ash border border-ash-medium text-text-strong rounded-full text-xs font-semibold shadow-sm">
                    {vendor.paymentNumber || 'N/A'}
                </span>
            </div>
            ...
            <div className="col-span-1 flex justify-center gap-2">
                {/* <button
                    onClick={() => handleView(vendor._id!)}
                    className="p-2 cursor-pointer text-action-primary hover:bg-brand-ash rounded-lg transition-colors"
                    title="View vendor Order"
                >
                    <i className="fas fa-eye"></i>
                </button> */}
