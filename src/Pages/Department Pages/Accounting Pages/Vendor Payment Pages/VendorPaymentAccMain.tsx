...
const VendorPaymentAccMain = () => {
    ...
    const deleteVendorPaymentMutation = useDeletevendorpayments();
    ...
    useEffect(() => {
        ...
        const handleScroll = () => {
            ...
            if (scrollHeight - scrollTop - clientHeight < 100 && hasNextPage && !isFetchingNextPage) {
                console.log('📜 Loading more Vendor payments...');
                fetchNextPage();
            }
        };
    ...
    const handleDelete = async (vendorpaymentsId: string) => {
        try {
            await deleteVendorPaymentMutation.mutateAsync({ vendorpaymentsId });
            refetch();
            toast({ title: "Success", description: "Deleted successfully" });
        } catch (error: any) {
            toast({
                title: "Error",
                description: error?.response?.data?.message || error?.message || "Failed to delete the",
                variant: "destructive"
            });
        }
    ...
    const handleView = (vendorPayId: string) => {
        navigate(`vendorpaymentsingle/${vendorPayId}`);
    };
    ...
    // Get all vendorPayments from pages
    const vendorPayment = data?.pages.flatMap(page => page.data) || [];
...
export default VendorPaymentAccMain;