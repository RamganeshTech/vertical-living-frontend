...
// // src/components/Department/Accounting/vendorsAccounts/vendorsAccSingle.tsx
...
// import { useGetVendor, useUpdateVendor, useUpdateVendorDocument, type UpdateVendorPayload, type Vendor } from '../../../../apiList/Department Api/Accounting Api/vendorAccApi';
// import VendorAccountForm from './VendorAccountForm';
// import VendorMapLocation from '../../../../components/VendorMapLocation';
...
//     // Fetch vendors data
//     const { data: vendor, isLoading, isError, error, refetch } = useGetVendor(id) as { data: Vendor, isLoading: boolean, isError: boolean, error: any, refetch: any };
...
//     const handleUpdateVendor = async (data: UpdateVendorPayload) => {
//         try {
...
//             refetch();
//         } catch (error: any) {
//             toast({ title: "Error", description: error.message || 'Failed to update vendor' });
//             throw error;
//         }
//     };
...
//                     {(error as any)?.message || "Failed to load vendor"}
//                 </p>
...
//     // If in editing mode, use the VendorAccountForm component
//     if (isEditing) {
//         return (
//             <VendorAccountForm
//                 mode="update"
...
//                             }`}>
//                                 {vendor.vendorType === 'business' ? 'Business' : 'Individual'}
//                             </span>
//                             <span className="text-sm text-gray-500">
//                                 <i className="fas fa-calendar-alt mr-1"></i>
//                                 Created: {dateFormate(vendor.createdAt)}
//                             </span>
//                         </div> */}
//                     </div>
...
//                                     <i className={`fas ${vendor.vendorType === 'business' ? 'fa-building' : 'fa-user'} mr-2 text-blue-600`}></i>
//                                     {vendor.vendorType === 'business' ? 'Business' : 'Individual'}
//                                 </p>
//                             </div> */}
...
//                                         First Name
//                                     </label>
//                                     <p className="text-gray-900">{vendor.firstName || '-'}</p>
//                                 </div>
...
//                                         Last Name
//                                     </label>
//                                     <p className="text-gray-900">{vendor.lastName || '-'}</p>
//                                 </div> */}
...
//                                         Mobile Number
//                                     </label>
//                                     <p className="text-gray-900">{vendor.phone?.mobile || '-'}</p>
//                                 </div>
//                                 <div>
//                                     <label className="block text-sm font-medium text-gray-700 mb-2">
...
//                                         Work Number
//                                     </label>
//                                     <p className="text-gray-900">{vendor.phone?.work || '-'}</p>
//                                 </div>
//                             </div>
...
//                                     Vendor Language
//                                 </label>
//                                 <p className="text-gray-900">{vendor.vendorLanguage || '-'}</p>
//                             </div>


//                             <div>
//                                 <p className="text-sm text-gray-500 mb-1">Shop Location</p>

//                                 <VendorMapLocation
//                                     latitude={vendor.location?.latitude || 12.9716}
//                                     longitude={vendor.location?.longitude || 77.5946}
//                                     address={vendor.location?.address}
//                                     height="h-48" // Smaller height for sidebar
//                                     width="w-full"
//                                 />
//                             </div>
//                         </div>
...
//                                     Accounts Payable
//                                 </label>
//                                 <p className="text-gray-900">{vendor.accountsPayable || '-'}</p>
//                             </div>
...
//                                     {vendor.currency?.split('-')[0].trim() || 'INR'} {vendor.openingBalance || 0}
//                                 </p>
//                             </div>
...
//                                         {vendor.documents.some(file => file.type === "image") && (
...
//                                                         {vendor.documents.filter(f => f.type === "image").length}
//                                                     </span>
//                                                 </div>
//                                                 <ImageGalleryExample
//                                                     imageFiles={vendor.documents.filter(file => file.type === "image")}
//                                                     height={150}
//                                                     minWidth={150}
//                                                     maxWidth={200}
//                                                 />
...
//                                         {/* PDFs Section */}
//                                         {vendor.documents.some(file => file.type === "pdf") && (
//                                             <div className=' w-1/2'>
//                                                 <div className="flex items-center gap-2 mb-3">
//                                                     <i className="fas fa-file-pdf text-red-600"></i>
//                                                     <h4 className="font-semibold text-gray-800 text-sm">Documents</h4>
//                                                     <span className="text-xs bg-red-100 text-red-700 px-2 py-1 rounded-full">
//                                                         {vendor.documents.filter(f => f.type === "pdf").length}
//                                                     </span>
//                                                 </div>
...
//                                                             >
//                                                                 <div className="flex items-center space-x-3">
//                                                                     <i className={`fas ${file.type === 'pdf' ? 'fa-file-pdf text-red-500' : 'fa-file-image text-blue-500'} text-2xl`}></i>
//                                                                     <div>
//                                                                         <p className="text-sm font-medium text-gray-900">
//                                                                             {file.originalName || 'Document'}
//                                                                         </p>
//                                                                         <p className="text-xs text-gray-500">
//                                                                             {file.type.toUpperCase()} • {dateFormate(file?.uploadedAt!)}
//                                                                         </p>
//                                                                     </div>
//                                                                 </div>
//                                                                 <a
...
//                             {(!vendor.documents || vendor.documents.length === 0) && selectedFiles.length === 0 && (
...
const VendorAccSingle = () => {
    ...
    const handleUpdate = async (data: UpdateVendorPayload, files?: File[]) => {
        try {
            if (!id) return;
            ...
            throw error; // Throw so Form knows to stay in edit mode if needed
        }
    };
