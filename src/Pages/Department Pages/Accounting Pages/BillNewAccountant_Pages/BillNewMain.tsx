...
const BillNewMain = () => {
    ...
    return (
        <div className="h-full flex flex-col bg-brand-surface overflow-hidden">

            {/* --- HEADER --- */}
            <header className="flex justify-between items-center bg-brand-surface border-b border-ash-medium flex-shrink-0 p-4">
                <div>
                    <h1 className="text-2xl font-bold text-text-main flex items-center">
                        <i className="fas fa-file-pdf mr-3 text-action-primary"></i>
                        Bill Designs
                    </h1>
                    <Breadcrumb paths={paths} />
                ...
                <div className='gap-2 flex items-center'>
                    ...
                    {canCreate && (
                        <Button variant='dark' onClick={() => navigate(`create`)} className="hover:opacity-90">
                            <i className="fas fa-plus mr-2"></i> Create Bill
                        </Button>
                    )}

                    <Button variant="outline" onClick={() => navigate(`billtemplate`)} className="hover:opacity-90">
                        <i className="fas fa-file-invoice mr-2"></i> Bill Template
                    </Button>
            ...
            {isLoading ? (
                ...
                <div className="max-w-xl mx-auto mt-10 p-6 bg-brand-surface border border-action-danger rounded-lg shadow text-center">
                    <div className="text-action-danger font-semibold mb-2 text-xl">⚠️ Error Occurred</div>
                    <p className="text-action-danger mb-4">{(error as any)?.message || "Failed to load bills."}</p>
                    <Button onClick={() => refetch()} className="bg-action-danger text-brand-surface hover:opacity-90">Retry</Button>
                </div>
            ) : (
                <main className="flex flex-col xl:flex-row flex-1 overflow-hidden">

                    {/* --- LEFT SIDEBAR (FILTERS) --- */}
                    <div className="xl:w-80 flex-shrink-0 border-r border-ash-medium bg-brand-surface overflow-y-auto p-4">
                        <div className="flex items-center justify-between mb-6">
                            ...
                            {activeFiltersCount > 0 && (
                                <button onClick={clearFilters} className="text-xs text-action-primary hover:underline font-medium">
                                    Clear All
                                </button>
                            )}
                        </div>
                        ...
                        <div className="space-y-5">
                            {/* Search */}
                            <div>
                                <label className="block text-xs font-bold text-text-muted uppercase mb-1">Search</label>
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        value={filters.search}
                                        onChange={(e) => setFilters(prev => ({ ...prev, search: e.target.value }))}
                                        className="w-full pl-8 pr-3 py-2 border border-ash-medium bg-brand-surface text-text-main placeholder-text-soft rounded text-sm focus:ring-2 focus:ring-action-primary outline-none"
                                    />
                                </div>
                            </div>
                            ...
                            {/* Project ID */}
                            <div>
                                <label className="block text-xs font-bold text-text-muted uppercase mb-1">Project ID</label>
                                <input
                                    type="text"
                                    placeholder="Filter by Project ID"
                                    value={filters.projectId}
                                    onChange={(e) => setFilters(prev => ({ ...prev, projectId: e.target.value }))}
                                    className="w-full px-3 py-2 border border-ash-medium bg-brand-surface text-text-main placeholder-text-soft rounded text-sm focus:ring-2 focus:ring-action-primary outline-none"
                                />
                            </div>

                            {/* Date Range */}
                            <div>
                                <label className="block text-xs font-bold text-text-muted uppercase mb-1">Date Range</label>
                                <div className="space-y-2">
                                    <input
                                        type="date"
                                        className="w-full px-3 py-2 border border-ash-medium bg-brand-surface text-text-main rounded text-sm focus:ring-2 focus:ring-action-primary outline-none"
                                        value={filters.createdFromDate}
                                        onChange={(e) => setFilters(prev => ({ ...prev, createdFromDate: e.target.value }))}
                                    />
                                    <input
                            ...
                            {/* Sort */}
                            <div>
                                <label className="block text-xs font-bold text-text-muted uppercase mb-1">Sort By</label>
                                <div className="grid grid-cols-2 gap-2">
                                    <select
                                        value={filters.sortBy}
                                        onChange={(e) => setFilters(prev => ({ ...prev, sortBy: e.target.value }))}
                                        className="w-full px-2 py-2 border border-ash-medium rounded text-sm outline-none bg-brand-surface text-text-main focus:ring-2 focus:ring-action-primary"
                                    >
                                        <option value="createdAt">Created</option>
                                        <option value="billNumber">Bill #</option>
                                        <option value="customerName">Customer</option>
                    ...
                    {canList && (
                        <div
                            ref={scrollContainerRef}
                            className="flex-1 overflow-y-auto bg-brand-ash p-4"
                        >
                            {bills.length === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full text-center">
                                    <div className="w-24 h-24 bg-brand-surface border border-ash-medium rounded-full flex items-center justify-center mb-4 shadow-sm">
                                        <i className="fas fa-box-open text-4xl text-text-soft" />
                                    </div>
                                    <h3 className="text-lg font-semibold text-text-main">No Bills Found</h3>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4 gap-6">
                                        {bills.map((bill: any) => (
                                            <Card
                                                key={bill._id}
                                                onClick={() => handleCardClick(bill._id)}
                                                className="cursor-pointer bg-brand-surface hover:shadow-xl transition-all duration-300 border-l-4 border-l-action-success relative group"
                                            >
                                                <CardHeader className="pb-2">
                                                    <div className="flex justify-between items-start">
                                                        <div className="flex-1">
                                                            <h4 className="font-bold text-lg text-text-main truncate" title={bill.customerName}>
                                                                {bill.customerName || 'Unknown Customer'}
                                                            </h4>
                                                            <span className="text-xs text-text-muted font-mono bg-brand-ash px-2 py-0.5 rounded mt-1 inline-block border border-ash-medium">
                                                                {bill.billNumber}
                                                            </span>
                                                        </div>
                                                        {canDelete && (
                                                            <Button
                                                                variant="danger"
                                                                size="icon"
                                                                className="h-7 w-7 text-action-danger hover:bg-brand-ash"
                                                                onClick={(e) => handleDelete(e, bill._id)}
                                                            >
                                                                <i className="fas fa-trash text-xs"></i>
                                                            </Button>
                                                    ...
                                                    <CardDescription className="mt-2 text-xs text-text-muted">
                                                        <i className="far fa-calendar mr-1"></i>
                                                        {dateFormate(bill.createdAt)}
                                                    </CardDescription>
                                                </CardHeader>
                                                <CardContent>
                                                    <div className="flex gap-2 flex-wrap">
                                                        {bill.projectId && (
                                                            <Badge variant="secondary" className="text-[10px] bg-brand-ash text-text-main border border-ash-medium">
                                                                <i className="fas fa-project-diagram mr-1 text-text-muted"></i>
                                                                {bill.projectId.projectName || 'Project'}
                                                            </Badge>
                                                        )}
                                                    </div>
