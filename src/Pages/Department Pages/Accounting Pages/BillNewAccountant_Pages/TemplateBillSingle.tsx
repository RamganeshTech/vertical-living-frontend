  ...
  return (
        <>
            <div
                ...
                onContextMenu={(e) => handleContextMenu(e)}
                onClick={(e) => { e.stopPropagation(); onClick(); }}
                className={`group ${isSelected ? 'ring-2 ring-action-primary' : 'hover:ring-1 hover:ring-ash-medium'}`}
            >
                {/* DRAG HANDLE */}
                <div
                    {...listeners} {...attributes}
                    className={`absolute -top-3 -left-3 w-6 h-6 bg-brand-surface shadow rounded-full flex items-center justify-center cursor-move z-50 
                    ${isSelected ? 'flex' : 'hidden group-hover:flex'}`}
                >
                    <i className="fa-solid fa-arrows-up-down-left-right text-xs text-text-muted"></i>
                </div>

                {component.type === 'text' && (
                    <textarea
                        className="w-full h-full p-1 bg-transparent border-transparent hover:border-dotted hover:border-ash-medium outline-none focus:bg-brand-surface-hover cursor-text resize-none overflow-hidden"
                        value={component.label}
                        style={{
                            fontSize: component.style.fontSize,
                            fontWeight: component.style.fontWeight as any,
                ...
                {component.type === 'data-field' && (
                    <div className="flex items-center gap-2 px-2 py-1 border border-dashed border-ash-dark bg-brand-ash w-full h-full overflow-hidden" {...listeners} {...attributes}>
                        <span className="font-bold text-xs whitespace-nowrap text-text-main">{component.label}:</span>
                        <input className="bg-transparent border-b border-ash-medium text-text-main text-xs w-full outline-none placeholder-text-soft" value={component.value} onChange={(e) => onUpdateValue(e.target.value)} placeholder="Value..." onPointerDown={(e) => e.stopPropagation()} />
                    </div>
                )}

                {component.type === 'image' && (
                    <div className="w-full h-full bg-brand-ash">
                        {Array.isArray(component.value) && component.value.length > 0 ? (
                            <img
                                src={
                                    typeof component.value[0] === 'string'
                            ...
                            <div className="w-full h-full flex items-center justify-center text-xs text-text-soft border border-dashed border-ash-medium">No Img</div>
                        )}
                    </div>
                )}
                ...
                {component.type === 'table' && Array.isArray(component.value) && (
                    <div className="w-full h-full flex flex-col relative">
                        ...
                        {isSelected && (
                            <div className="flex w-full h-2 absolute -top-2 left-0 pointer-events-none z-20">
                                {component.columnWidths?.map((w, i) => (
                                    <div key={i} style={{ width: `${w}%` }} className="relative border-r border-action-primary h-2 pointer-events-auto">
                                        <div onMouseDown={(e) => handleColResize(e, i)} className="absolute right-[-4px] top-0 w-2 h-4 cursor-col-resize bg-action-primary hover:bg-action-primary-hover rounded-sm z-50"></div>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        {isSelected && (
                            <div className="absolute -left-2 top-0 w-2 h-full flex flex-col pointer-events-none z-20">
                                {component.rowHeights?.map((h, i) => (
                                    <div key={i} style={{ height: `${h}px` }} className="relative border-b border-action-primary w-2 pointer-events-auto">
                                        <div onMouseDown={(e) => handleRowResize(e, i)} className="absolute bottom-[-4px] left-0 w-4 h-2 cursor-row-resize bg-action-primary hover:bg-action-primary-hover rounded-sm z-50"></div>
                                    </div>
                                ))}
                            </div>
                        )}
                        ...
                        <table
                            ...
                            style={{
                                borderCollapse: 'collapse',
                                border: '1px solid #000' // Outer border kept as #000 for PDF styling
                            }}
                        >
                            <colgroup>
                                {component.columnWidths?.map((w, i) => <col key={i} style={{ width: `${w}%` }} />)}
                                        ...
                                        {row.map((cell: TableCellData, c: number) => {
                                            ...
                                            return (
                                                <td
                                                    key={c}
                                                    style={{
                                                        border: '1px solid #000', // Inner cell borders kept for PDF style
                                                        padding: 0,
                                                        verticalAlign: 'top',
                                                        backgroundColor: cellStyle.backgroundColor || 'transparent',
                                                    }}
                                                    className={`relative ${isActiveCell ? 'ring-2 ring-inset ring-action-primary z-10' : ''}`}
                                                    onContextMenu={(e) => handleContextMenu(e, r, c)}
                                                >
                                                    <textarea
                                                        className="w-full h-full p-1 border-none outline-none bg-transparent cursor-text resize-none overflow-hidden whitespace-pre-wrap text-text-main"
                                                        style={{
                                                            fontSize: `${cellStyle.fontSize || component.style.fontSize}px`,
                                                            color: cellStyle.color || component.style.color,
                                                            textAlign: (cellStyle.textAlign || component.style.textAlign) as any,
                ...
                {isSelected && (
                    <div
                        onMouseDown={handleResizeMouseDown}
                        onPointerDown={(e) => e.stopPropagation()}
                        className="absolute -bottom-1 -right-1 w-4 h-4 bg-action-primary cursor-nwse-resize rounded-full z-50 border-2 border-brand-surface shadow"
                    ></div>
                )}
            </div>
            ...
            {contextMenu && (
                <ContextMenu
                    ...
                    onClose={() => setContextMenu(null)}
                    actions={[
                        { label: 'Insert Row Below', action: insertRow, color: 'text-action-primary' },
                        { label: 'Insert Column Right', action: insertCol, color: 'text-action-primary' },
                        { label: 'Delete This Row', action: deleteRow, color: 'text-text-main' },
                        { label: 'Delete This Column', action: deleteCol, color: 'text-text-main' },
                        { label: 'Delete Entire Table', action: onDelete, color: 'text-action-danger font-bold border-t border-ash-medium mt-1 pt-1' }
                    ]}
                />
            )}
        </>
...
const PropertySidebar = ({
    ...
    const renderStyleControls = (
        ...
        <div className="space-y-2 p-3 bg-brand-ash rounded border border-ash-medium">
            <h4 className="text-[10px] font-bold text-text-muted uppercase">{labelPrefix} Styling</h4>
            <div className="grid grid-cols-3 gap-2">
                <div>
                    <span className="text-[10px] text-text-soft">Size</span>
                    <input 
                        type="number" 
                        className="w-full border border-ash-medium bg-brand-surface text-text-main p-1 text-xs rounded outline-none focus:ring-1 focus:ring-action-primary" 
                        value={fontSize || 12} 
                        onChange={(e) => onChange('fontSize', Number(e.target.value))} 
                    />
                </div>
                <div className="relative">
                    <span className="text-[10px] text-text-soft">Text</span>
                    <div className="flex items-center border border-ash-medium p-1 rounded bg-brand-surface h-7 focus-within:ring-1 focus-within:ring-action-primary">
                        <input 
                            type="color" 
                ...
                <div className="relative">
                    <span className="text-[10px] text-text-soft">BG Color</span>
                    <div className="flex items-center border border-ash-medium p-1 rounded bg-brand-surface h-7 focus-within:ring-1 focus-within:ring-action-primary">
                        <input 
                            type="color" 
                            className="w-full h-full border-none p-0 bg-transparent cursor-pointer" 
            ...
            <div className="flex gap-1">
                <button 
                    ...
                    className={`flex-1 border p-1 rounded text-xs transition-colors ${fontWeight === 'bold' ? 'bg-action-primary text-brand-surface border-action-primary' : 'bg-brand-surface text-text-main border-ash-medium'}`}
                >
                    <i className="fa-solid fa-bold"></i>
                </button>
                <button 
                    onClick={() => onChange('textAlign', 'left')} 
                    className={`flex-1 border p-1 rounded text-xs transition-colors ${textAlign === 'left' ? 'bg-action-primary text-brand-surface border-action-primary' : 'bg-brand-surface text-text-main border-ash-medium'}`}
                >
                    <i className="fa-solid fa-align-left"></i>
                </button>
                <button 
                    onClick={() => onChange('textAlign', 'center')} 
                    className={`flex-1 border p-1 rounded text-xs transition-colors ${textAlign === 'center' ? 'bg-action-primary text-brand-surface border-action-primary' : 'bg-brand-surface text-text-main border-ash-medium'}`}
                >
                    <i className="fa-solid fa-align-center"></i>
                </button>
                <button 
                    onClick={() => onChange('textAlign', 'right')} 
                    className={`flex-1 border p-1 rounded text-xs transition-colors ${textAlign === 'right' ? 'bg-action-primary text-brand-surface border-action-primary' : 'bg-brand-surface text-text-main border-ash-medium'}`}
                >
    ...
    return (
        <div className="w-72 bg-brand-surface border-l border-ash-medium flex flex-col h-full shadow-xl z-50 fixed right-0 top-16 bottom-0" onClick={(e) => e.stopPropagation()}>
            <div className="p-4 border-b border-ash-medium bg-brand-ash flex justify-between items-center">
                <span className="font-bold text-xs uppercase text-text-main">Properties</span>
                <button onClick={onClose}><i className="fa-solid fa-xmark text-text-soft hover:text-text-main transition-colors"></i></button>
            </div>
            <div className="p-4 space-y-6 overflow-y-auto flex-1">
                
                {component.type === 'text' && (
                    <div>
                        <label className="text-xs font-bold text-text-soft mb-1 block">Text Content</label>
                        <textarea 
                            className="w-full border border-ash-medium bg-brand-surface text-text-main p-2 text-sm rounded outline-none focus:ring-2 focus:ring-action-primary" 
                            rows={3} 
                            value={component.label} 
                ...
                {component.type === 'data-field' && (
                    <div>
                        <label className="text-xs font-bold text-text-soft mb-1 block">Config</label>
                        <input 
                            className="w-full border border-ash-medium bg-brand-surface text-text-main placeholder-text-soft p-2 text-sm rounded mb-2 outline-none focus:ring-2 focus:ring-action-primary" 
                            placeholder="Label" 
                            value={component.label} 
                            onChange={(e) => onUpdate('label', e.target.value)} 
                        />
                        <input 
                            className="w-full border border-ash-medium bg-brand-surface text-text-main placeholder-text-soft p-2 text-sm rounded outline-none focus:ring-2 focus:ring-action-primary" 
                ...
                {component.type === 'table' && activeCell && (
                    <div className="mb-4 animate-in fade-in slide-in-from-right-4 duration-300">
                        <div className="flex items-center gap-2 mb-2">
                            <span className="w-2 h-2 rounded-full bg-action-success"></span>
                            <span className="text-xs font-bold text-action-success">Selected Cell ({activeCell.r + 1}, {activeCell.c + 1})</span>
                        </div>
                        {renderStyleControls(
                            cellStyle.fontSize || component.style.fontSize,
                            cellStyle.color || component.style.color,
                        ...
                        <hr className="my-4 border-ash-medium" />
                    </div>
                )}
                ...
                <hr className="border-ash-medium" />
                <div className="grid grid-cols-2 gap-2">
                    <div>
                        <span className="text-[10px] text-text-soft">X</span> 
                        <input 
                            type="number" 
                            className="w-full border border-ash-medium bg-brand-ash text-text-muted p-1 text-sm rounded cursor-not-allowed outline-none" 
                            value={Math.round(component.x)} 
                            readOnly 
                        />
                    </div>
                    <div>
                        <span className="text-[10px] text-text-soft">Y</span> 
                        <input 
                            type="number" 
                            className="w-full border border-ash-medium bg-brand-ash text-text-muted p-1 text-sm rounded cursor-not-allowed outline-none" 
                ...
                <button 
                    onClick={() => onDelete()} 
                    className="w-full mt-6 bg-brand-surface hover:bg-brand-ash text-action-danger py-2 rounded border border-action-danger text-sm transition-colors"
                >
                    Delete Block
                </button>
            </div>
...
export const TemplateBillSingle: React.FC<TemplateBillSingleProps> = ({ mode }) => {
    ...
    return (
        <div className="h-screen flex flex-col bg-brand-ash overflow-hidden">
            {/* TOOLBAR */}
            <div className="h-14 bg-brand-surface border-b border-ash-medium flex items-center justify-between px-4 shadow-sm z-20">
                <div className="flex items-center gap-4">
                    <button onClick={() => navigate(-1)}>
                        <i className="fa-solid fa-arrow-left text-text-muted hover:text-text-main transition-colors"></i>
                    </button>
                    <input 
                        className="font-bold text-text-main bg-transparent outline-none placeholder-text-soft" 
                        value={templateName} 
                ...
                {(canCreate || canEdit) &&  <Button onClick={handleSave} variant="dark" isLoading={createMutation.isPending || updateMutation.isPending}>Save Template</Button> }
            </div>

            {/* CANVAS WRAPPER */}
            <div className="flex-1 bg-ash-light overflow-auto p-8 relative" onClick={() => { setSelectedId(null); setActiveCell(null); }}>
                <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
                    {/* ACTUAL A4 PAPER CANVAS */}
