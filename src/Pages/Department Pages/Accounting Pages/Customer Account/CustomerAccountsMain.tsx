...
const CustomerAccountsMain = () => {
    ...
    const handleDelete = async (customerId: string,) => {
        try {
            ...
            toast({
                title: "Error",
                description: error?.response?.data?.message || error?.message || "Failed to delete the customer",
                variant: "destructive"
            });
        }
    ...
    return (
        <div className="space-y-2 h-full bg-brand-surface">
            {/* Header */}
            <header className="flex justify-between items-center">
                <div>
                    <h1 className="text-3xl font-bold text-text-main flex items-center">
                        <i className="fas fa-users mr-3 text-action-primary"></i>
                        Customer Accounts
                    </h1>
                    <Breadcrumb paths={paths} />
                </div>
                    ...
                    {canList && <>
                        {customers.length === 0 ? (
                            <div className="flex flex-col items-center justify-center min-h-[300px] w-full bg-brand-surface border border-ash-medium rounded-xl text-center p-6 shadow-sm">
                                <i className="fas fa-user-slash text-5xl text-text-soft mb-4" />
                                <h3 className="text-lg font-semibold text-text-main mb-1">No Customers Found</h3>
                                <p className="text-sm text-text-muted">
                                    {filters.search
                                        ? 'Try adjusting your filters to find customers.'
                                        : 'Looks like there are no customers yet.'}
                                    <br />
                                    Click on <strong className="text-text-strong">"Add Customer"</strong> to get started 🚀
                                </p>
                            </div>
                        ) : (
                            <div
                            ...
                            >
                                ...
                                <div className="bg-brand-surface rounded-t-xl border border-ash-medium sticky top-0 z-10">
                                    <div className="grid grid-cols-14 gap-4 px-6 py-4 bg-brand-surface-hover border-b border-ash-medium font-semibold text-text-strong text-sm">
                                        <div className="col-span-1 text-center">S.No</div>
                                        <div className="col-span-3 text-center">Customer Name</div>
                                        <div className="col-span-2 text-center">Company Name</div>
                                        <div className="col-span-2 text-center">Email</div>
