...
import { useGetInstantCostMain, useUpsertInstantCostMain } from "../../apiList/Quote Api/RateConfig Api/instantCostCalculator_Api/instantCostCalculatorMainApi";
...
const InstantCostCalculation: React.FC = () => {
    ...
    // --- 4. LABOUR STATE ---
    const [labour, setLabour] = useState({ categoryId: "", categoryName: "", ratePerDay: 0, noOfPersons: 1, noOfDays: 1 });
    ...
    // import { useGetInstantCostMain, useUpsertInstantCostMain } from "../../Hooks/useInstantCostMainHooks";

    // Inside your component:
    const { data: configData, isLoading: isFetching } = useGetInstantCostMain(organizationId!);
    const { mutateAsync: upsertConfig, isPending: isSaving } = useUpsertInstantCostMain();
            ...
            if (configData) {
                ...
                const formatMaterialArr = (arr: any[], defaultCatName: string, isFittings = false) => {
                    ...
                    return arr.map(item => {
                        ...
                        let extractedThickness = item.thickness || "";

                        if (typeof bId === 'object' && bId !== null) {
                            safeBrandId = bId._id;
                            const d = bId.data || {};
                            extractedBrandName = (d.Brand ?? d.BrandName ?? d.brand ?? d.brandName) || extractedBrandName;

                            // ⚠️ FIX: Extract thickness from the populated data if Mongoose stripped it!
                        if (!extractedThickness) {
                            extractedThickness = String(d["thickness (mm)"] || d.thickness || d.Thickness || "");
                        }


                            if (isFittings && bId.categoryName) extractedCatName = bId.categoryName;
                        } else {
                            safeBrandId = bId;
                        }
                        ...
                        return {
                            ...
                            thickness: extractedThickness // Make sure thickness is applied!
                        };
                    });
                };

                // Restore Product Dimensions
                if (configData.dimension) {
                    setProductDim({ width: configData.dimension.width || 0, height: configData.dimension.height || 0 });
                }

                // Restore Materials
                // if (configData.plywood?.length > 0) setPlywoods(configData.plywood);
                // 1. Plywood (FIXED: Now safely formatted to prevent [object Object])
                if (configData.plywood?.length > 0) {
                    setPlywoods(formatMaterialArr(configData.plywood, "Plywood"));
                }
                if (configData.finishes?.laminate?.inner?.length > 0) setInnerLaminates(formatMaterialArr(configData.finishes.laminate.inner, "Inner Laminate"));
                if (configData.finishes?.laminate?.outer?.length > 0) setOuterLaminates(formatMaterialArr(configData.finishes.laminate.outer, "Outer Laminate"));

                if (configData.finishes?.pu?.length > 0) { setOtherFinishes(formatMaterialArr(configData.finishes.pu, "PU Finish")); setActiveFinish('PU'); }
                else if (configData.finishes?.du?.length > 0) { setOtherFinishes(formatMaterialArr(configData.finishes.du, "DUCO Finish")); setActiveFinish('DUCO'); }
                else if (configData.finishes?.paint?.length > 0) { setOtherFinishes(formatMaterialArr(configData.finishes.paint, "Paint Finish")); setActiveFinish('Paint'); }
                else if (configData.finishes?.varnish?.length > 0) { setOtherFinishes(formatMaterialArr(configData.finishes.varnish, "Varnish Finish")); setActiveFinish('Varnish'); }
                else { setActiveFinish('Laminate'); }
                ...
                if (configData.fittings?.length > 0) {
                    ...
                    formattedFittings.forEach((fit: any, i: number) => {
                        if (fit.categoryName) fetchAllCategoryItems(i, fit.categoryName);
                    });
                }

                // Restore Nails & Glues (Mapping itemName -> name)
                if (configData.nailsAndGlues?.length > 0) {
                    setEssentials(configData.nailsAndGlues.map((n: any) => ({
                        name: n.itemName || n.name || "",
                        rate: n.rate || 0,
    ...
    // --- CALCULATIONS ---
    const safeNum = (val: any) => Math.max(Number(val), 0) || 0;
    ...
    const labourTotal = safeNum(labour.ratePerDay) * safeNum(labour.noOfPersons) * safeNum(labour.noOfDays);
    ...
    const handleReset = () => {
        if (window.confirm("Are you sure you want to clear all calculator values? This cannot be undone.")) {
            ...
            // ⚠️ FIX: Reset Labour to the default first category
            const defaultCatId = allLabourCategory.length > 0 ? allLabourCategory[0]._id : "";
            const defaultCatName = allLabourCategory.length > 0 ? allLabourCategory[0].name : "";
            
            setLabour({ 
                categoryId: defaultCatId, 
                categoryName: defaultCatName,
                ratePerDay: 0, // This will auto-update via your existing useEffect once the ID changes!
                noOfPersons: 1, 
                noOfDays: 1 
            });
    ...
    const handleSave = async () => {
        ...
        const cleanArr = (arr: any[]) => arr.filter((item, i) => i !== arr.length - 1 || Number(item.rate) > 0 || Number(item.quantity) > 1 || item.brandName || item.name || item.itemName || item.categoryName);

        const payload = {
            organizationId,
            // Provide a dummy ID if your generic calculator doesn't use a specific category, 
            // OR update your Mongoose Schema to make categoryId false for the generic model.
            // categoryId: "000000000000000000000000",
            dimension: productDim, // Saving the width/height

            plywood: cleanArr(plywoods),
            finishes: {
                laminate: {
            ...
            // Map frontend 'name' back to backend 'itemName'
            nailsAndGlues: cleanArr(essentials).map((e: any) => ({
                itemName: e.name,
                rate: e.rate,
                quantity: e.quantity,
                totalCost: (Number(e.rate) || 0) * (Number(e.quantity) || 0)
            })),

            labour: {
                categoryId: labour.categoryId,
                rate: labour.ratePerDay,
                noOfDays: labour.noOfDays,
                noOfPersons: labour.noOfPersons,
                totalCost: labourTotal
            },
            fabrication: {
                sqftRate: factory.ratePerSqft,
                areaSqft: factory.totalSqft,
                totalCost: factoryTotal
            },
        ...
        <div className="min-h-screen bg-brand-surface px-4 custom-scrollbar overflow-y-auto pb-24 w-full">
            ...
            <header className="flex items-center justify-between mb-6 pb-4 border-b border-ash-light">
                ...
                <div className="flex items-end gap-3 w-full sm:w-auto">
                    ...
                    <Button onClick={handleSave} isLoading={isSaving} variant="dark"
                        className="h-10 px-8 shadow-sm font-black text-xs uppercase tracking-widest">
                        ...
                        <Card className="bg-brand-surface border border-ash-medium shadow-sm rounded-xl overflow-hidden h-fit">
                            ...
                            <CardContent className="p-4 grid grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <Input
                                        type="number"
                                        value={labour.noOfPersons || ""}
                                        onChange={(e) => setLabour({ ...labour, noOfPersons: safeNum(e.target.value) })}
