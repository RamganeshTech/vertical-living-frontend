...
//         filters.startDate,
//         filters.endDate,
//     ].filter(Boolean).length + (filters.minAmount > 0 || filters.maxAmount < 9000000 ? 1 : 0);
...
//                                                 <span className={`px-3 py-1.5 rounded-lg text-[9px] font-extrabold uppercase border ${item.finish === 'Basic' ? 'bg-indigo-50 text-indigo-600 border-indigo-100' :
...
const PublicCostCalculationMain = () => {
    ...
    const [isFilterOpen, setIsFilterOpen] = useState(false); // New state for mobile filter toggle
    const [filters, setFilters] = useState({
        search: "",
        finish: "",
        startDate: "",
    ...
    return (
        <div className="w-full bg-gray-50/30 h-full">
            {/* Header kept completely unchanged as requested */}
            <header className="flex justify-between items-center mb-8">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 flex items-center uppercase tracking-tight">
                        <i className="fa fa-calculator mr-3 text-blue-600"></i>
            ...
            <main className="flex gap-6 h-[88vh] flex-col md:flex-row relative">

                {/* --- Mobile Filter Button (Only visible on small screens) --- */}
                <div className="md:hidden flex justify-between items-center bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex-shrink-0">
                    <span className="text-sm font-bold text-gray-700">Filter Leads</span>
                    <button
                ...
                <div className={`
                    ...
                    bg-white md:rounded-3xl p-6 shadow-2xl md:shadow-sm border-r md:border border-gray-100
                `}>
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-sm font-bold uppercase tracking-widest text-gray-500">Filters</h3>
                        <div className="flex items-center gap-4">
                            ...
                            <button onClick={() => setIsFilterOpen(false)} className="md:hidden text-gray-400 hover:text-gray-700">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="pb-8"> {/* Added padding bottom to prevent slider cutoff */}
                            <label className="block text-[10px] font-bold uppercase text-gray-400 mb-2">Budget Range (Estimate)</label>
                            <div className="px-2 pt-4">
                                <Slider
                                    range
                ...
                {/* --- Table View --- */}
                <div className="flex-1 bg-white md:rounded-[35px] rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full">
                    {isLoading ? (
                        <div className="flex-1 flex items-center justify-center"><i className="fa fa-spinner fa-spin text-[#ffc000] text-2xl" /></div>
                    ) : (
                        <div className="flex-1 overflow-auto relative custom-scrollbar">
                            ...
                            <table className="w-full text-left border-collapse whitespace-nowrap">
                                <thead className="sticky top-0 z-10 bg-slate-50 border-b border-slate-200">
                                    <tr className="text-[10px] uppercase tracking-[2px] text-slate-500">
                                        <th className="px-6 py-5 font-extrabold">S.No</th>
                                        <th className="px-6 py-5 font-extrabold">Date</th>
                                        <th className="px-6 py-5 font-extrabold">Client Details</th>
                                        <th className="px-6 py-5 font-extrabold">Property</th>
                                        <th className="px-6 py-5 font-extrabold text-center">Finish</th>
                                        <th className="px-6 py-5 font-extrabold text-right">Estimate</th>
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white">
                                    {records.map((item: any, idx: number) => (
                                        ...
                                        >
                                            {/* 1. S.No */}
                                            <td className="px-6 py-5 text-xs font-extrabold text-blue-600/40 group-hover:text-blue-600">
                                                {idx + 1}
                                            </td>

                                            {/* 2. Date */}
                                            <td className="px-6 py-5 text-xs font-bold text-slate-700">
                                                {dateFormate(item.createdAt)} <br />
                                            ...
                                            <td className="px-6 py-5 text-right">
                                                <span className="text-blue-600 font-bold text-[10px] mr-1">₹</span>
                                                <span className="font-bold text-slate-900 text-[15px]">
                                                    {item.estimate?.toLocaleString('en-IN') || "0"}
                                                </span>
                                            </td>
