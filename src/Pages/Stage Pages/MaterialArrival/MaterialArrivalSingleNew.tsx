...
// const MaterialArrivalCard: React.FC<MaterialArrivalCardProps> = ({ item, projectId }) => {
...
//     } catch (error: any) {
//       toast({ title: "Error", description: error?.response?.data?.message || error.message || "failed to updated", variant:"destructive"});
//     }
//   };


//   console.log("oage;ljefl", item)

//   return (
//     <div className="grid grid-cols-5 items-center gap-2 px-4 py-3 border-b bg-white text-sm">
//       <div className="truncate text-gray-800 font-medium">{item.customId || "N/A"}</div>
...
//       <div>
//         <span
//           className={`inline-block px-2 py-1 text-xs rounded-full font-semibold ${item.isVerified ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}`}
//         >
...
//           disabled={isPending}
//           className={`text-xs px-3 py-1 ${item.isVerified ? "bg-green-600 hover:bg-green-700" : "bg-gray-400 hover:bg-gray-500"} text-white`}
//         >
