...
import { useCommonOrderHistoryGneratePdf, useCommonOrderHistorySendToProcurement, useGetSingleCommonOrderItem } from "../../../apiList/Stage Api/Common OrderMaterialHisotry APi/commonOrderHistoryMaterialApi";

const CommonOrderItemSingle = () => {
    const { id, orderItemId, organizationId } = useParams<{ id: string; orderItemId: string, organizationId: string }>();
    ...
    // --- API Hooks ---
    const { data: orderData, isLoading, isError, refetch } = useGetSingleCommonOrderItem(id!, orderItemId!);
    ...
    const handleGeneratePdf = async () => {
        try {
            const res = await generateLink({ id: id!, organizationId: organizationId!, orderItemId:orderItemId! });
...
export default CommonOrderItemSingle;