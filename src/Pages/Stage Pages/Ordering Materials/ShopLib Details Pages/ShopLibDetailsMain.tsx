...
import { Button } from "../../../../components/ui/Button";
import { useAuthCheck } from "../../../../Hooks/useAuthCheck";
import { useGetAllVendors } from "../../../../apiList/Department Api/Accounting Api/vendorAccApi";
...
const ShopLibDetailsMain: React.FC = () => {
    ...
    const { role, permission } = useAuthCheck();
    // Permissions remain tied to ordermaterial or updated to vendor permissions as per your requirements
    // const canDelete = role === "owner" || permission?.ordermaterial?.delete;
    const canCreate = role === "owner" || permission?.ordermaterial?.create;

    // Integrated Infinite Scroll Hook for Vendors
    const {
        data,
        isLoading,
        error,
    ...
    // Flatten the paginated data into a single array for rendering
    const allVendors = useMemo(() => {
        return data?.pages.flatMap((page) => page.data) || [];
    }, [data]);

    const handleViewShop = (vendorId: string) => {
        // Navigating to the consolidated vendor library details
        navigate(`single/${vendorId}`);
    };
    ...
    return (
        <div className="min-h-full overflow-y-auto bg-gray-50 p-4 md:p-6">
            <div className="max-w-full mx-auto">
                ...
                <header className="bg-white rounded-lg shadow-sm p-6 mb-6">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                        <div className="flex gap-2">
                            ...
                            <div>
                                <h1 className="text-2xl md:text-3xl font-bold text-gray-800">
                                    <i className="fas fa-warehouse mr-3 text-blue-500"></i>
                                    Vendor Library
                                </h1>
                                <p className="text-gray-600 mt-1">Consolidated vendor and shop management</p>
                            </div>
                        </div>
                        {canCreate && (
                            <Button
                                onClick={() => navigate("/vendors/add")} // Navigate to your unified vendor creation
                    ...
                    <div className="mt-6 flex flex-col md:flex-row gap-4">
                        <div className="flex-1 relative">
                            ...
                            <input
                                type="text"
                                placeholder="Search by shop name, vendor or category..."
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                        ...
                        <div className="flex gap-2">
                            <button onClick={() => setViewMode("grid")} className={`px-4 py-2 rounded-lg ${viewMode === "grid" ? "bg-blue-500 text-white" : "bg-gray-200"}`}>
                                <i className="fas fa-th-large"></i>
                            </button>
                            <button onClick={() => setViewMode("table")} className={`px-4 py-2 rounded-lg ${viewMode === "table" ? "bg-blue-500 text-white" : "bg-gray-200"}`}>
                                <i className="fas fa-list"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {allVendors.map((vendor: any) => (
                            <div
                                ...
                                onClick={() => handleViewShop(vendor._id)}
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <div className="flex-1">
                                        <h3 className="text-lg font-semibold text-blue-700 mb-1">
                                            {vendor.shopDisplayName || "Unnamed Shop"}
                                        </h3>
                                        <span className="text-xs bg-blue-50 text-blue-600 px-2 py-1 rounded-full uppercase font-bold">
                                            {vendor.vendorCategory || "General"}
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-2 mt-4">
                                    ...
                                    <div className="flex items-center text-sm text-gray-600">
                                        ...
                                        <span>{vendor.phone?.mobile || "No Phone"}</span>
                                    </div>
                                    {vendor.shopFullAddress && (
                                        <div className="flex items-center text-sm text-gray-600">
                                            <i className="fas fa-map-marker-alt w-5 text-gray-400"></i>
                                            <span className="truncate">{vendor.shopFullAddress}</span>
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                {/* Table View omitted for brevity - follow same pattern as grid */}
                ...
                {hasNextPage && (
                    <div className="mt-8 flex justify-center">
                        ...
                        >
                            {isFetchingNextPage ? "Loading more..." : "Load More Vendors"}
                        </Button>
                    </div>
                )}
