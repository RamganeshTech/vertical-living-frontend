...
// Interface for strictly typing our filters
...
const fetchAllOrganizations = async (api: AxiosInstance, filters?: OrgFilters) => {
  // Construct the query string dynamically based on active filters
  const params = new URLSearchParams();
  
  if (filters?.search) params.append('search', filters.search);
  if (filters?.type) params.append('type', filters.type);
  if (filters?.planType) params.append('planType', filters.planType);
  if (filters?.planStatus) params.append('planStatus', filters.planStatus);

  const queryString = params.toString();
  const endpoint = `/orgs/getallorganizations${queryString ? `?${queryString}` : ''}`;
...
const updateOrganizationLogo = async ({
  ...
  // "logo" must match imageUploadToS3.single("logo") in your backend route
  formData.append("logo", file);

  const { data } = await api.put(`/orgs/updatelogo/${orgId}`, formData, {
    headers: {
...
export const useGetAllOrganizations = (filters?: OrgFilters) => {
  const allowedRoles = ["owner", "staff", "CTO"];
  const { role } = useGetRole();
  const api = getApiForRole(role!);

  return useQuery({
    // Adding `filters` to the queryKey ensures React Query caches different 
    // filtered states separately and refetches when filters change.
    queryKey: ["myOrgs-all", filters], 
    queryFn: async () => {
      if (!role) throw new Error("Not Authorized");
      if (!allowedRoles.includes(role)) throw new Error("Not allowed to make this API call");
      if (!api) throw new Error("API instance not found for role");

      return await fetchAllOrganizations(api, filters);
    },
    refetchOnWindowFocus: false,
    retry: false,
    // Only run the query if we have successfully initialized the role and API instance
    enabled: !!role && !!api, 
  });
};
...
export const useUpdateOrganizationLogo = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (!role) throw new Error("Not authorized");
      if (!allowedRoles.includes(role)) throw new Error("You are not allowed to access this API");
      if (!api) throw new Error("API instance is null");

      return await updateOrganizationLogo({ file, orgId, api });
    },
    onSuccess: () => {
      // Invalidate both lists and single view to ensure the new logo shows up everywhere
      queryClient.invalidateQueries({ queryKey: ["myOrgs"] });
      queryClient.invalidateQueries({ queryKey: ["myOrgs-single"] });
    },
  });
